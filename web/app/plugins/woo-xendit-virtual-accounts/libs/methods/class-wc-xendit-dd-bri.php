...
class WC_Xendit_DD_BRI extends WC_Xendit_Invoice {
    ...
    const DEFAULT_PAYMENT_DESCRIPTION = 'Bayar pesanan dengan akun Direct Debit anda melalui <strong>Xendit</strong>';
    ...
    public function __construct() {
		...
		$this->method_title = __('Xendit Direct Debit', 'woocommerce-gateway-xendit');
        $this->method_description = sprintf(__('Collect payment from Direct Debit on checkout page and get the report realtime on your Xendit Dashboard. <a href="%1$s" target="_blank">Sign In</a> or <a href="%2$s" target="_blank">sign up</a> on Xendit and integrate with <a href="%3$s" target="_blank">your Xendit keys</a>.', 'woocommerce-gateway-xendit' ), 'https://dashboard.xendit.co/auth/login', 'https://dashboard.xendit.co/register', 'https://dashboard.xendit.co/settings/developers#api-keys');
